...
%Segunda retracción------------------
    ...
    La figura \ref{fig:diagrama1} reperesenta la situación actual, los ejes corresponden a los planos $u^{\lambda+1}=\dots=u^n=0$ y $u^{1}=\dots=u^{\lambda}=0$. La circunferencia representa la frontera de la bola de radio $\sqrt{e\varepsilon}$ y centro $p$. La región más oscura se corresponde con $M^{c-\varepsilon}_f$; la región con puntos grandes es $f^{-1}[c-\varepsilon,c]$ y por último, la región con los puntos más pequeños es $f^{-1}[c,c+\varepsilon]$.
    ...
    Asimismo, como $\frac{\partial F}{\partial \eta}>0$ (es decir, $F$ crece en la dirección de $\eta$) se tiene que $F(D(t,q))\le F(q)$ para todo $t\in [0,1]$ y todo $q\in R_1$. Luego $D$ lleva $M_f^{c-\varepsilon_0}\cup H$ en sí mismo.
    \bigskip
...
\begin{proof}
    ...
    \end{equation*}
    Además, está bien definida ya que envía representantes de la misma clase de equivalencia al mismo punto. En $X\cup_{\varphi_0} e^{\lambda}$ están relacionados $u$ y $\varphi_0(u)$ para todo $u$ en $\partial e^{\lambda}$ y $k$ verifica que 
    \begin{equation*}
...
\begin{proof}
    ...
    En primer lugar, que el conjunto $\{c_i\}_{i\in \mathcal{C}}$ tenga un punto de acumulación $c$ implica que existe una sucesión $\{c_{i_k}\}_{k\in \mathbb{N}}$ convergente a un valor $c$. Ahora, por esto, existen inifnitos $c_i$ menores que $c+1$ y como, por definición, para cada $c_i$ existe un punto crítico $p_i\in M$ tal que $f(p_i) = c_i$ tenemos que $M^{c+1}$ contiene infinitos puntos críticos de $f$. 
