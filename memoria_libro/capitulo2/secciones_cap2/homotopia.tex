...
\begin{definition}
    ...
    \begin{enumerate}[label=\roman*)]
        \item Para todo $t\in \mathbb{R},~~ \varphi_t:M\to M$ definida como $\varphi_t(q)=\varphi(t,q)$, es un difeomorfismo de $M$ sobre si misma.

        \item Para todos $t, s\in\mathbb{R}$ se tiene que $\varphi_{t+s}=\varphi_t\circ \varphi_s$.
    \end{enumerate}
\end{definition}

\bigskip

Dado un grupo uniparamétrico de difeomorfismos $\varphi$ definimos el siguiente campo vectorial sobre $M$
\begin{equation*}
...
\begin{proof}
    ...
    Ahora, por el teorema de Picard-Lindelöf sabemos que existe localmente una solución única, que depende de manera diferenciable de la condición inicial. Es decir, para todo $ p\in M$ existen un entorno $\mathcal{U}$ de $p$ y $\varepsilon > 0$ tales que $(\ast)$ tiene una única solución para todo $q\in \mathcal{U}$ y $|t|<\varepsilon$.

    Como $K\subset M$ es compacto, puede ser cubierto por una cantidad finita de dichos entornos. Sea $\varepsilon_0$ el menor de los valores definidos arriba y establezcamos $\varphi_t(q)=q,~~\forall q\notin K$ (luego $X_q=0$ fuera de $K$). Entonces $(\ast)$ tiene una única solución $\varphi_t(q)$ para $|t|<\varepsilon_0$ y para todo $q\in M$ suave respecto de ambas variables.
...
\begin{proof}
    ...
    \end{equation*}
     ...
     Para concluir la demostración falta comprobar que efectivamente $M^a$ es retracto de deformación de $M^b$. Para ello vamos a construir una homotopía relativa a $M^a$ de la identidad de $M^b$ a una retracción de $M^b$ a $M^a$.
...
\end{proof}
\begin{remark}
    Nótese la importancia de que $f^{-1}[a,b]$ sea compacto. Sean $a=-1/2$ y $b=1/2$ y nuestra variedad la esfera $\mathbb{S}^2$ embebida en $\mathbb{R}^3$, es claro que la función que asigna a cada punto de la esfera su coordenada $z$ es de Morse y verifica que $f^{-1}[a,b]$ es compacto puesto que es cerrado y la esfera es compacta. Ahora, si eliminamos de $\mathbb{S}^2$ un punto $p$ tal que $f(p)$ queda entre $f(a)$ y $f(b)$ se tiene que $f^{-1}[a,b]$ deja de ser compacto (pues deja de ser cerrado y estamos en un espacio Hausdorff) y claramente $M^a$ no es retracto de deformación de $M^b$. En efecto, $M^a$ es homeomorfo a un disco y $M^b$ a un disco perforado, que no es simplemente conexo, luego no tienen el mismo tipo de homotopía.
\end{remark}
\bigskip
...
\begin{theorem}
\label{pegadocelula}
     Sea $f:M\to\mathbb{R}$ suave sobre una variedad riemanniana $(M,\langle\cdot,\cdot\rangle)$ y $p\in M$ un punto crítico no degenerado de $f$ con índice $\lambda$. Supongamos que $f(p)=c$ y que existe $\varepsilon>0$ tal que $f^{-1}[c-\varepsilon,c+\varepsilon]$ es compacto y no contiene ningún punto crítico de $f$ a parte de $p$. Entonces, para todo $\varepsilon>0$ suficientemente pequeño, $M^{c+\varepsilon}_f$ tiene el tipo de homotopía de $M^{c-\varepsilon}_f$ con una $\lambda$-celda pegada.
\end{theorem}
...
\begin{proof}
    ...
    Después, probaremos que $M^{c-\varepsilon}_f$ con una $\lambda$-celda pegada es retracto de deformacion de $M^{c-\varepsilon}_F$ y por la transitividad de esta relación habremos terminado.
...
%Preliminares------------------
    ...
    \begin{equation*}
        f(q) = c - \xi(q) + \eta(q)
    \end{equation*}
    para cada $q\in \mathcal{U}$, donde $c=f(p).$

    Sea $\varepsilon>0$ suficientemente pequeño para que:
    \begin{enumerate}[label=\roman*)]
        \item $f^{-1}[c-\varepsilon,c+\varepsilon]$ sea compacto y no contenga ningún punto crítico de $f$ distinto de $p$.
        \item $\phi(\mathcal{U})\subset \mathbb{R}^n$ contenga a la bola cerrada
        \begin{equation*}
...
%Construcción de F----------------
    ...
    Es claro que $F$ esta bien definida ya que $\mu$ se anula en $\partial\mathcal{U}$. Además, es suave ya que en un entorno de $\partial\mathcal{U}$ se tiene que $\mu'$ es idénticamente $0$, luego tanto aproximándonos desde dentro de $\mathcal{U}$ como desde fuera $d_qF=d_qf$.

    \bigskip
    
%Primera retraccion-------------------------------

    Ahora que tenemos $F$ debemos comprobar que nos encontramos en las hipótesis del teorema \ref{retractodef}. Veamos primero que $M_F^{c+\varepsilon}=M_f^{c+\varepsilon}$, después que $F$ y $f$ poseen los mismos puntos críticos y que $F^{-1}[c-\varepsilon,c+\varepsilon]$ no contiene ningún punto crítico.

    En efecto, $\mu(\xi(q)+2\eta(q))=0$ para todo $q\in M$ tal que $\xi(q)+2\eta(q)\ge 2\varepsilon$ luego fuera del elipsoide $E=\{q\in M:\xi(q)+2\eta(q)\le 2\varepsilon\}$ se tiene que $F=f$.

    \medskip
    
    Sabemos que $E\subset M_f^{c+\varepsilon}$ y por
    \begin{equation*}
        F(q)<f(q)=c-\xi(q)+\eta(q)\le c+\frac{1}{2}\xi(q)+\eta\le c + \varepsilon, ~\forall q\in E,
    \end{equation*}

    se tiene que $E\subset M_F^{c+\varepsilon}$. Visto que es la única región en la que difieren concluimos que $M_F^{c+\varepsilon}=M_f^{c+\varepsilon}$.
    ...
    Veamos que $F^{-1}[c-\varepsilon,c+\varepsilon]=M^{c+\varepsilon}_F\setminus M^{c-\varepsilon}_F$ es compacto y no contiene ningún punto crítico. Es claro que $M^{c-\varepsilon}_f\subset M^{c-\varepsilon}_F$ ya que $F$ y $f$ coinciden salvo en el entorno de $p$ en el que $F\le f$ así
    \begin{equation*}
    ...
    \end{equation*}
    por lo que $F^{-1}[c-\varepsilon,c+\varepsilon]$ es compacto. Por otro lado,
    \begin{equation*}
    ...
    De esta manera, nos encontramos en las hipótesis del teorema \ref{retractodef} para $F$, lo que demuestra que $M^{c-\varepsilon}_F$ es retracto de deformación de $M^{c+\varepsilon}_F=M^{c+\varepsilon}_f$.

    \bigskip

%Segunda retracción------------------

    Sea $e^{\lambda}=\{q\in \mathcal{U}: \xi(q)\le \varepsilon,~\eta(q)=0 \}\subset \mathcal{U}$ una $\lambda$-celda. En primer lugar, veamos que
    \begin{equation*}
        \partial e^{\lambda}=\{q\in \mathcal{U}: \xi(q)= \varepsilon,~~\eta(q)=0 \}=M^{c-\varepsilon}_f\cap e^{\lambda}.
    \end{equation*}
    En efecto:
    \begin{quote}
    $\left( \subseteq \right) ~~\text{Para todo }~ q\in \partial e^{\lambda},~f(q)=c-\varepsilon \implies q\in M^{c-\varepsilon}_f$.
    
    $\left( \supseteq \right) ~~\text{Para todo }~ q\in M^{c-\varepsilon}_f\cap e^{\lambda}$, se tiene $\xi(q)\le\varepsilon,~\eta(q)=0 \text{ y } f(q)\le -\varepsilon$, entonces como
    \begin{equation*}
        f(q)=c-\xi(q)\le c-\varepsilon\iff \xi(q)\ge \varepsilon
    \end{equation*}
    \quad \quad necesariamente $\xi(q)=\varepsilon$, luego $q\in \partial e^{\lambda}$. 
    \end{quote}
    \bigskip

    La Figura \ref{fig:diagrama1} muestra los conjuntos subnivel de $f$ y la $\lambda$-celda. Los ejes corresponden a los planos $u^{\lambda+1}=\dots=u^n=0$ y $u^{1}=\dots=u^{\lambda}=0$. La circunferencia representa la frontera de la bola de radio $\sqrt{2\varepsilon}$ y centro $p$. La región más oscura se corresponde con $M^{c-\varepsilon}_f$, la región con puntos grandes es $f^{-1}[c-\varepsilon,c]$ y, por último, la región con los puntos más pequeños es $f^{-1}[c,c+\varepsilon]$.

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\textwidth]{/home/kike/Documents/Mates/TFG/introduction-to-morse-theory/memoria_libro/imagenes/diagrama1_def.pdf}
        \caption{Representación esquemática del entorno del punto crítico $p$ respecto de $f$.}
        \label{fig:diagrama1}
    \end{figure}
    ...
    De cara al resto de la demostración resultará conveniente introducir la noción de \textit{asa}. Entenderemos $M_F^{c-\varepsilon}$ como $M_f^{c-\varepsilon}\cup H$ donde $H=\overline{M_F^{c-\varepsilon}\setminus M_f^{c-\varepsilon}}$ es el asa del que hablamos, denotado así en virtud de su nombre en inglés \textit{handle}.
    ...
    \begin{equation*}
        F(q)\le F(p)<c-\varepsilon ~\text{ y }~f(q)=c-\xi(q)\ge c-\varepsilon,~\forall q\in e^{\lambda},
    \end{equation*}
    ...
    Ahora queremos demostrar que $M^{c-\varepsilon}_f\cup e^{\lambda}$ es retracto de deformación de $M^{c-\varepsilon}_F = M_f^{c-\varepsilon}\cup H$. Para ello vamos a construir una homotopía relativa a $M^{c-\varepsilon}_f\cup e^{\lambda}$ de la identidad a una retracción en $M^{c-\varepsilon}_f\cup e^{\lambda}$, como en la demostración del teorema \ref{retractodef}. 

    La Figura \ref{fig:diagrama_asa} muestra el entorno de $p$ desde esta perspectiva. La zona oscura representa $M_f^{c-\varepsilon}$, la zona punteada se corresponde con $F^{-1}[c-\varepsilon,c+\varepsilon]=M^{c+\varepsilon}_F\setminus M^{c-\varepsilon}_F$ y la zona con flechas es $H$, el asa. Además de identificar $H$, las flechas ilustran la deformación que haremos en el siguiente y último paso de la demostración.


    \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\textwidth]{/home/kike/Documents/Mates/TFG/introduction-to-morse-theory/memoria_libro/imagenes/diagrama2_asa.pdf}
        \caption{Representación esquemática del entorno del punto crítico $p$ respecto de $F$ y con el asa.}
        \label{fig:diagrama_asa}
    \end{figure}

    \bigskip

    Sea $\Psi:[0,1]\times M_f^{c-\varepsilon}\cup H\to M_f^{c-\varepsilon}\cup H$, definimos $\Psi$ como la identidad fuera de $\mathcal{U}$. En $\mathcal{U}$ debemos distinguir tres regiones.

    La Figura \ref{fig:diagrama_asa_regiones} muestra de manera esquemática las regiones en las que queda divido $\mathcal{U}$ cerca del asa. La zona con las flechas de punta negra representa la parte de la región $1$ que queda dentro del asa. La zona con las flechas pequeñas de punta hueca coincide con la parte de la región $2$ que queda dentro del asa. La zona oscura es la región $3$. Asimismo, las flechas ilustran qué parte del asa queremos deformar y sobre qué. Nótese que en la región $3$ no es necesario deformar nada.
    \medskip

    \textbf{Región 1:}\quad $R_1=\{q\in\mathcal{U}:\xi(q)\le \varepsilon\}$.
    
    En esta región lo que queremos es llevar los puntos a $e^{\lambda}$.

    Definimos $\Psi(t,q)=\Psi(t,(u_1(q),\dots,u_n(q))=(u_1(q),\dots,u_{\lambda}(q),tu_{\lambda+1}(q),\dots,tu_n(q))$.
    \bigskip
    
    Es claro que es relativa a $M^{c-\varepsilon}_f\cup e^{\lambda}$ ($R_1\cap M^{c-\varepsilon}_f=\varnothing$ y los puntos de $e^{\lambda}$ no los toca) y que  $\Psi(1,\cdot)$ es la identidad. Además,
    \begin{equation*}
        \forall q\in R_1, ~\Psi(0, q)\in e^{\lambda} ~\text{ y }~ \Psi(0,\Psi(0,q))=\Psi(0,q).
    \end{equation*} 

    Asimismo, como $\frac{\partial F}{\partial \eta}>0$ (es decir, $F$ crece en la dirección de $\eta$) se tiene que $F(\Psi(t,q))\le F(q)$ para todo $t\in [0,1]$ y todo $q\in R_1$. Luego $\Psi$ lleva $M_f^{c-\varepsilon}\cup H$ en sí mismo.
    \bigskip

    \textbf{Región 2:}\quad $R_2=\{q\in \mathcal{U}:~ \varepsilon\le \xi(q)\le \eta(q)+\varepsilon\}$.


    Aquí lo que buscamos es llevar los puntos de $R_2$ a la frontera de $M_f^{c-\varepsilon}$.

    Definimos $\Psi(t,q)=\Psi(t,(u_1(q),\dots,u_n(q))=(u_1(q),\dots,u_{\lambda}(q),s_t(q) u_{\lambda+1}(q),\dots,s_t(q) u_n(q))$, donde 
    \begin{equation*}
        s_t(q)=t + (1-t)\left(\frac{\xi(q)-\varepsilon}{\eta}\right)^{1/2} \in ~[0,1]
    \end{equation*}

    Igual que sobre $R_1$ queda claro que es relativa a $M_f^{c-\varepsilon}\cup e^{\lambda}$, que $D(1,\cdot)$ es de nuevo la identidad y que $\Psi(0,\cdot)$ es una retracción sobre la frontera de $M_f^{c-\varepsilon}$. Lo último se sigue de que en la frontera de $M_f^{c-\varepsilon}$ se tiene que $-\xi +\eta = \varepsilon$, luego $s_0(q)=1$. 
    
    Resta ver que $s_t$ es continua cuando $\xi \to \varepsilon$ y $\eta \to 0$. Reordenando la desigualdad que define $R_2$ tenemos $0\le\xi-\varepsilon\le\eta$, lo que significa que a medida que $\eta$ tiende a cero, $\xi$ debe tender a $\varepsilon$ más rápidamente. Así, 
    \begin{equation*}
        0\le \lim_{\xi \to \varepsilon, \eta\to 0} \frac{\xi - \varepsilon}{\eta} \le 1
    \end{equation*}
    y en particular, se tiene que $\lim_{\xi \to \varepsilon, \eta\to 0} \frac{\xi - \varepsilon}{\eta}=0$ necesariamente. Concluimos entonces que $s_t$ no genera problemas en la continuidad de $\Psi$ cerca de los puntos con $\eta(q)=0$.
    \bigskip

    \textbf{Región 3:}\quad $R_3=\{q\in \mathcal{U}:~ \eta(q) + \varepsilon \le \xi(q)\}$
    

    $R_3$ coincide con $M_f^{c-\varepsilon}$ luego basta con tomar $\Psi(t,q)= Id_{M_f^{c-\varepsilon}}$.
    \bigskip

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{/home/kike/Documents/Mates/TFG/introduction-to-morse-theory/memoria_libro/imagenes/diagrama_asa_regiones.pdf}
        \caption{Representación esquemática de las regiones en las que queda dividida el asa.}
        \label{fig:diagrama_asa_regiones}
    \end{figure}


    Las tres regiones cubren por completo $\mathcal{U}$. Esto concluye la demostración de que $M^{c-\varepsilon}_f\cup e^{\lambda}$ es retracto de deformación de $M^{c-\varepsilon}_F = M_f^{c-\varepsilon}\cup H$ y con ello la demostración del teorema.
...
\label{obs_incluso_con_c_critico}
    \hfill
    \begin{enumerate}[label=(\roman*)]
        \item Modificando un poco la demostración es posible demostrar que $M^c_f$ es también retracto de deformación de $M^{c+\varepsilon}_f$. De hecho, $M^c_f$ es retracto de deformación de $M_F^c$, que es a su vez retracto de deformación de $M^{c+\varepsilon}_f$, esto, junto con el teorema, deja claro que $M^{c-\varepsilon}_f\cup e^{\lambda}$ es retracto de deformación de $M^c_f$.
        \item Además, como corolario de esta prueba se sigue que $M^{c+\varepsilon}_f$ no solo es homotópicamente equivalente sino que es homeomorfo a $M^{c-\varepsilon}$ con una $\lambda$-asa $D^\lambda \times D^{n-\lambda}$ pegada. En caso de ser $M$ compacta, hacer esto en cada punto crítico da lugar a una descomposición de $M$ en pegados sucesivos de asas que se conoce como ``handlebody decomposition''. Esta descomposición es la que se utiliza, por ejemplo, en la demostración del teorema del h-cobordismo.
    \end{enumerate}
...
A continuación enunciamos el caso general de este resultado, cuya demostración es similar.
...
\begin{definition}
    Se dice que una función $f:X\to Y$ entre CW-complejos es una \textit{función celular} si $f(X_k)\subset Y_k$ para todo $k$. Es decir, una función que lleva celdas a celdas de su misma dimensión o más pequeña.
\end{definition}
...
\begin{definition}
    ...
    \begin{equation*}
        \Delta X := \text{min}\{\dim K\colon K \text{ es un CW-complejo que domina a }X\}.
    \end{equation*}
\end{definition}
...
\label{lema1_democwvariedad}
    Sean $\varphi_0, \varphi_1: \partial D^{\lambda} \to X$ dos funciones de pegado homotópicas. Entonces, la función identidad de $X$ se puede extender a una equivalencia homotópica
    \begin{equation*}
...
\begin{proof}
    ...
    Sea $\{\varphi_t:\partial D^{\lambda} \to X\}$ una homotopía entre $\varphi_0$ y $\varphi_1$, definimos $k$ como sigue.
    \begin{equation*}
        \begin{cases}
            k(x) = x &~~\text{ si }~~ x\in X\\
            k(tu) = 2tu &~~\text{ si }~~ 0\le t\le \frac{1}{2},~u\in \partial D^{\lambda}\\
            k(tu) = \varphi_{2-2t}(u) &~~\text{ si }~~  \frac{1}{2} \le t\le 1,~u\in \partial D^{\lambda},
        \end{cases}
    \end{equation*}
    donde $tu$ denota el producto usual de el escalar $t\in [0,1]$ con el vector unitario $u\in \partial D^{\lambda}$, es decir, cubre todo $D^{\lambda}$. Es continua ya que para $t=\frac{1}{2}$ se tiene
    \begin{equation*}
        k(u/2) = u = \varphi_{1}(u) ~~\text{ y }~~ u \sim \varphi_{1}(u) ~\text{ en }~ X\cup_{\varphi_1} e^{\lambda}.
    \end{equation*}
    Además, está bien definida ya que envía representantes de la misma clase de equivalencia al mismo punto. En $X\cup_{\varphi_0} e^{\lambda}$ están relacionados $u$ y $\varphi_0(u)$ para todo $u$ en $\partial D^{\lambda}$ y $k$ verifica que 
    \begin{equation*}
    ...
    \begin{equation*}
        \begin{cases}
            l(x) = x &~~\text{ si }~~ x\in X,\\
            l(tu) = 2tu &~~\text{ si }~~ 0\le t\le \frac{1}{2},~u\in \partial D^{\lambda},\\
            l(tu) = \varphi_{2t-1}(u) &~~\text{ si }~~  \frac{1}{2} \le t\le 1,~u\in \partial D^{\lambda}.
        \end{cases}
    \end{equation*}
    ...
    \begin{equation*}
        \begin{cases}
            ...
            l\circ k(tu) = 4tu &~~\text{ si }~~ 0\le t\le \frac{1}{4},~u\in \partial D^{\lambda},\\
            l\circ k(tu) = \varphi_{4t-1}(u) &~~\text{ si }~~  \frac{1}{4} \le t\le \frac{1}{2},~u\in \partial D^{\lambda},\\
            l\circ k(tu) = \varphi_{2-2t}(u) &~~\text{ si }~~  \frac{1}{2} \le t\le 1,~u\in \partial D^{\lambda}.
        \end{cases}
    \end{equation*}
    ...
    \begin{cases}
        ...
        h_s(tu) = (4-3s)tu &~~\text{ si }~~ 0 \le t \le \frac{1}{4-3s},~u\in \partial D^{\lambda} \\[4pt]
        h_s(tu) = \varphi_{(4-3s)t-1}(u) &~~\text{ si }~~ \frac{1}{4-3s} \le t \le \frac{2-s}{4-3s},~u\in \partial D^{\lambda} \\[4pt]
        h_s(tu) = \varphi_{\frac{1}{2}(4-3s)(1-t)}(u),
        &~~\text{ si }~~  \frac{2-s}{4-3s} \le t \le 1,~u\in \partial D^{\lambda}.
    \end{cases}
    \end{equation*}
...
\label{lema2_democwvariedad}
    Sea $\varphi: \partial D^{\lambda} \to X$ una función de pegado. Toda equivalencia homotópica $f:X\to Y$ se puede extender a una equivalencia homotópica 
    \begin{equation*}
...
\begin{proof}
    ...
    \begin{equation*}
        \begin{cases}
            ...
            kGF(tu) = 2tu &~~\text{ si }~~ 0\le t\le \frac{1}{2},~u\in \partial D^{\lambda},\\
            kGF(tu) = h_{2-2t}(\varphi(u)) &~~\text{ si }~~ \frac{1}{2}\le t\le 1,~u\in \partial D^{\lambda}.
        \end{cases}
    \end{equation*} 
    ...
    \begin{equation*}
        \begin{cases}
            ...
            q_s(tu) = \frac{2}{1+s}tu &~~\text{ si }~~ 0\le t\le \frac{1+s}{2},~u\in \partial D^{\lambda},\\
            q_s(tu) = h_{2-2t+s}(\varphi(u)) &~~\text{ si }~~ \frac{1+s}{2}\le t\le 1,~u\in \partial D^{\lambda}.
        \end{cases}
    \end{equation*}

    En $s=0$ recuperamos $kGF$ ya que $h_0\equiv g\circ f$, los otros dos tramos son claros. En $s=1$ se tienen $h_1$, que es la identidad en $X$,  $q_1(tu)=tu$ para todo $t\in[0,1]$ y $q_1(u)=h_1(\varphi(u))$ en el último tramo (ya que $t$ solo puede tomar el valor $1$), que es igual a $\varphi(u)$ que está relacionado con $u$. Se comprueba fácilmente que no hay problemas de continuidad observando que los valores que toma $q$ cuando $t=\frac{1+s}{2}$ están relacionados. Además, $q$ está bien definida ya que $q_s(u) = h_s(\varphi(u)) = q_s(\varphi(u))$.

    Así hemos probado que $F$ tiene una inversa homotópica por la izquierda. Se prueba de manera análoga que $G$ tiene un inversa homotópica por la derecha. Ahora debemos probar que $kG$ es, en efecto, también una inversa homotópica por la derecha de $F$. Para ello probamos el siguiente resultado primero:
    \begin{quote}
    ...
    Ahora, como $k(GF)\simeq \text{id}_{X\cup_{\varphi} e^{\lambda}}$ y sabemos que $k$ tiene una inversa homotópica por la izquierda, se tiene que $(GF)k\simeq \text{id}_{X\cup_{g\circ f\circ\varphi} e^{\lambda}}$. Donde hemos aplicado el desarrollo de la demostración del resultado intermedio con $l=L$, $F=k$ y $R=GF$.
    ...
    Por último, como $F(kG) \simeq \text{id}_{Y\cup_{f\circ\varphi} e^{\lambda}}$ y $F$ tiene una inversa homotópica por la izquierda, se tiene que $F$ es una equivalencia homotópica, lo que conlcluye la demostración.
\end{proof}
...
\end{theorem}
\begin{proof}
    Sea $\{c_i\}_{i\in \mathcal{C}}$ el conjunto de los valores críticos de $f$ ordenados de tal manera que se verifica $c_1<c_2<c_3<\cdots$. Este conjunto no puede tener ningún punto de acumulación, veámoslo.

    En primer lugar, que el conjunto $\{c_i\}_{i\in \mathcal{C}}$ tenga un punto de acumulación $c$ implica que existe una sucesión $\{c_{i_k}\}_{k\in \mathbb{N}}$ convergente a un valor $c$. Ahora, por esto, existen infinitos $c_i$ menores que $c+1$ y como, por definición, para cada $c_i$ existe un punto crítico $p_i\in M$ tal que $f(p_i) = c_i$ tenemos que $M^{c+1}$ contiene infinitos puntos críticos de $f$. 
    
    Por la compacidad de $M^{c+1}$, esta colección infinita de puntos críticos contiene necesariamente una sucesión $\{p_{i_k}\}_{k\in \mathbb{N}}$ convergente a un punto $p$ de $M^{c+1}$. Por continuidad de $f$ se tiene que 
    \begin{equation*}
    ...
    Sea $a\in\mathbb{R}\setminus\{c_i\}_{i\in \mathcal{C}}$, procedemos ahora por inducción en la cantidad de valores críticos menores que $a$. 
    ...
    \[
        ...
        \end{array}
    \]
    tales que cada una extiende a la anterior. Sea $K=\bigcup_{i=1}^{\infty} K_i$ con la mínima topología que hace continuas cada una de las inclusiones $i_j:K_j\hookrightarrow K$. Es decir, la topología tal que $\mathcal{U}\subseteq K$ es abierto si y solo si para todo $j\in \mathbb{N}$ se tiene que $i_j^{-1}(\mathcal{U})$ es abierto en $K_j$. 
    
    Definimos la función límite $\Phi:M\to K$ como la función que lleva un punto $x\in M$ al punto $i_i\circ \varphi_i(x)\in K$. En primer lugar, está bien definida ya que no importa la elección de $\varphi_i(x)$. En efecto, $x$ pertenece a todos los $M^{a_j}$ a partir de un cierto $j\in\mathbb{N}$ y, como cada equivalencia homotópica extiende a la anterior, la imagen de $x$ por $\varphi_j$ es la misma independientemente del índice $j$ elegido.
    
    Además, es continua ya que, dado $\mathcal{U}\subseteq K$ abierto, se verifica
    \begin{equation*}
        \Phi^{-1}(\mathcal{U})= (i_i\circ \varphi_i)^{-1}(\mathcal{U})= \varphi_i^{-1}\circ i_i^{-1}(\mathcal{U}) \overset{\text{\scriptsize abierto}}{\subseteq} M,
    \end{equation*}
    donde la última inclusión se sigue de la continuidad de $i_i$ y $\varphi_i$. Se tiene que $\Phi$ induce isomorfismos entre los grupos de homotopía en todas las dimensiones, luego, por el teorema \ref{dominados}, es una equivalencia homotópica, lo que concluye la demostración.

\end{proof}
\begin{remark}
    Al final de la demostración, para ver que se puede aplicar el teorema \ref{dominados} hay que comprobar que tanto $K$ como $M$ están dominados por CW-complejos. Es claro que $K$ está dominado por sí mismo. Para demostrar que $M$ lo está, basta observar que es retracto de deformación de un entorno tubular suyo, el cual es, a su vez, difeomorfo a un abierto de $\mathbb{R}^m$ para cierto $m$, y todo abierto de $\mathbb{R}^m$ tiene el tipo de homotopía de un CW-complejo.
\end{remark}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\subsection{Aplicaciones de los teoremas sobre el pegado de $\lambda$-celdas}\label{subsec:aplicaciones}
Presentamos brevemente dos ejemplos de aplicaciones de los teoremas acerca del pegado de $\lambda$-celdas.

\begin{theorem}[Teorema de Reeb]\label{reeb}
...
\begin{proof}
    ...
    Por otro lado, como $f^{-1}[\varepsilon, 1-\varepsilon]$ no contiene ningún punto crítico, por el teorema \ref{retractodef} $M^{\varepsilon}$ y $M^{1-\varepsilon}$ son difeomorfos. Así, $M$ es igual a la unión de los dos $n$-discos $M^{1-\varepsilon}$ y $f^{-1}[1-\varepsilon,1]$ pegados a lo largo de su frontera común, que es homeomorfo a una esfera.
...
\begin{remark}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item Utilizando el teorema \ref{cwvariedad} es fácil ver que $M$ tiene el tipo de homotopía de un CW-complejo con una $0$-celda y una $n$-celda. Una esfera admite una estructura de CW-complejo con esas celdas, sin embargo, el teorema no nos dice cómo se pegan. Luego no podemos asegurar que $M$ tenga el tipo de homotopía de la esfera, lo que sería suficiente para demostrar el resultado gracias a la \textit{conjetura de Poincaré generalizada}.
        \item No es necesario que $M$ sea difeomorfa a la esfera. Esto se debe a la existencia de \textit{estructuras exóticas} en algunas dimensiones: espacios homeomorfos a la esfera cuyas estructuras diferenciables no son difeomorfas a la de la esfera estándar.
    \end{enumerate}
\end{remark}

\newpage

Otra aplicación de los teoremas de la sección anterior es la siguiente. Sea $\mathbb{CP}^n$ el espacio proyectivo complejo de dimensión $n$. Pensaremos en $\mathbb{CP}^n$ como el espacio cuyos puntos son las clases de equivalencia de $(n+1)$-uplas $(z_0,\dots,z_n)$ de números complejos denotadas por $[z_0:z_1:\dots:z_n]$ tales que $\sum_j |z_j|^2=1$. 

Definimos la función $f:\mathbb{CP}^n \to \mathbb{R}$ dada por 
\begin{equation*}
    f[z_0:z_1:\dots:z_n] = \sum_{j=0}^n c_j |z_j|^2,
\end{equation*}
donde $c_0,\dots,c_n\in \mathbb{R}$ son constantes distintas dos a dos. Para hallar sus puntos críticos consideremos el siguiente sistema de coordenadas locales de $\mathbb{CP}^n$. Sea $\mathcal{U}_0$ el subespacio de los puntos tales que $z_0$ es distinto de $0$ y sea 
\begin{equation*}
...
Dado que $\text{H}_p f$ es una matriz diagonal cuyos elementos son constantes no nulas, $p_0$ es no degenerado. Además, es sencillo ver que su índice es dos veces el número de constantes $c_j$ que son menores que $c_0$.
...
\begin{remark}
    La descomposición de $\mathbb{CP}^n$ que acabamos de obtener recupera la noción de que $\mathbb{CP}^n$ es $\mathbb{C}^{n}$ con un $\mathbb{CP}^{n-1}$ pegado en el infinito. Veámoslo:

    Sea $H=\{p=[z_0:z_1:\dots:z_n]\in \mathbb{CP}^n ~|~ z_0=0\}$, entnces $\mathbb{CP}^n\setminus H$ contiene los puntos cuya primera coordenada es no nula, es decir, aquellos pertenecientes a las clases con representantes $[1:z_1:\dots:z_n]$. Así, se tiene que $\mathbb{CP}^n\setminus H$ es homeomorfo a $\mathbb{C}^n$ que es homeomorfo a una $2n$-celda. Ahora, $H$ es, a su vez, homeomorfo a $\mathbb{CP}^{n-1}$. Por lo que $\mathbb{CP}^n$ es el resultado de pegar una $2n$-celda a $H$. Iterando este proceso se llega a la descomposición en celdas que habíamos hallado.
\end{remark}