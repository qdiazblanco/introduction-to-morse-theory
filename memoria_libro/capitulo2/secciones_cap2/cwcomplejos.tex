...
\begin{proposition}
    ...
    \begin{equation*}
        X_n/X_{n-1} = \vee_{\alpha\in\Lambda_n} D^n_{\alpha}/\partial D^n_{\alpha} = \vee_{\alpha\in\Lambda_n} \mathbb{S}^{n-1}_{\alpha},
    \end{equation*}
    donde $\vee$ denota la suma wedge \cite[p.~10]{hatcher2002} y $\Lambda_n$ recorre las $n$-celdas de $X$.
\end{proposition}

Se sigue de que cada $n$-celda $e_{\alpha}^n$ se obtiene pegando la frontera de un $n$-disco al $n-1$-esqueleto. Al cocientar $X_n/X_{n-1}$ estamos colapsando a un solo punto el $n-1$-esqueleto, de manera que componiendo la función de pegado con la de paso al cociente lo que se obtiene es una clase trivial, luego la frontera del disco queda identificada a un solo punto y se tiene que 
\begin{equation*}
    D^n/\partial D^n \cong \mathbb{S}^n.
\end{equation*}
...
\begin{equation*}
    \vee_{\alpha\in\Lambda_n} \mathbb{S}^{n-1}_{\alpha}
\end{equation*}