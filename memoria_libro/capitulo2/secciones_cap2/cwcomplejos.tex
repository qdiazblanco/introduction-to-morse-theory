...
%tipo de homotopia-----------------------
Diremos que dos funciones continuas $f, g:X\to Y$ entre dos espacios topológicos son \textit{homotópicas} si existe una \textit{homotopía} entre ellas, es decir, si existe una función continua $H:X\times [0,1]\to Y$ que verifica
\begin{equation*}
    H(x,0)=f(x) ~~\text{ y }~~ H(x,1)=g(x)
\end{equation*}
para todo $x\in X$. 

La relación de homotopía es una relación de equivalencia. Denotaremos que dos funciones están relacionadas mediante el símbolo $\simeq$.

A la hora de denotar o construir homotopías usaremos indistintamente dos notaciones. Nos referimos a una homotopía entre dos funciones continuas $f$ y $g$ como la función continua $H(x,t): X\times [0,1]\to Y$ que verifica las condiciones anteriores o como la familia de funciones continuas $\{H_t: X\to Y\}_{t\in [0,1]}$ tales que $H_t(x)=H(x,t)$ para cualesquiera $t$ y $x$.
...
Una \textit{retracción} de un espacio $X$ en un subespacio $A\subseteq X$ es una aplicación continua $r:X\to A$ que verifica que $r|_A= \text{id}_A$, en ese caso, llamamos a $A$ \textit{retracto} de $X$. Además, una homotopía $\{H_t\}$ se dice \textit{relativa a A} si $H_t|_A=\text{id}_A$ para todo $t\in[0,1]$.
...
%CW-complejos-------------------------------
En lo que sigue denotaremos por $D^n$ el disco unidad cerrado, cuya frontera es $\partial D^n=\mathbb{S}^{n-1}$, la esfera unidad $(n-1)$-dimensional.

\begin{definition}
    Una \textit{celda} $e^n$ es un espacio topológico homeomorfo al interior del disco unidad $n$-dimensional $D^n$. Diremos que la dimensión de la celda es $n$ y que $e^n$ es una \textit{n-celda}.
\end{definition}
...
\begin{remark} \hfill
    \begin{enumerate}
        \item Nótese que de las funciones de pegado solo se exige que sean continuas.
        \item Decimos que la dimensión de un CW-complejo $X$ es $m$ si la construcción tiene una cantidad finita $m$ de pasos, en ese caso, $X=X_m$. Si la cantidad de pasos es infinita, el CW-complejo tendrá dimensión infinita. 
...
Remitimos al lector a la seccion \ref{subsec:homcelular} sobre homología celular, en la que desarrollamos algunos ejemplos de CW-complejos.
