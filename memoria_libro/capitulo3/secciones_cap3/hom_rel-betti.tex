...
\label{escision}
    Sea $X$ un espacio topológico y $A$ y $Z$ subespacios tales que $Z\subset A\subset X$ y la adherencia de $Z$ está contenida en $A$. Entonces, la inclusión $(X\setminus Z, A\setminus Z)\hookrightarrow (X,A)$, induce isomorfismos $H_n((X\setminus Z, A\setminus Z))\to H_n(X,A)$ para todo $n$. {\color{red}igual quitamos esta version}

    Equivalentemente, para subespacios $A,B\subset X$ cuyos interiores rescubren $X$, la inclusión $(B,A\cap B)\hookrightarrow (X,A)$ induce isomorfismos $H_n(B,A\cap B)\to H_n(X,A)$ para todo $n$.
\end{theorem}
