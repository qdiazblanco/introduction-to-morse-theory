...
Para eliminar posibles ambigüedades en el uso de ciertos términos, comenzamos la sección presentando de manera informal las variedades diferenciables y estableciendo la dirección de las cartas que será la utilizada en adelante. También conviene prestar atención a qué nos referimos cuando hablamos de sistema de coordenadas local y cómo lo denotamos.
...
\end{equation*}
son suaves para cualesquiera $\alpha$ y $\beta$.

En la presente sección todo espacio $M$ denotará una variedad diferenciable $n$-dimensional salvo que se especifique lo contrario.
...
Haremos abuso de la notación para refererirnos indistintamente como $f$ a una función $f:M\to \mathbb{R}$ y su expresión en coordenadas $\tilde{f} = f\circ\varphi_{\alpha}^{-1} : \mathbb{R}^n \to \mathbb{R}$. Es decir, hablaremos de la imagen de un punto $p$ por $f$ como $f(p)$ y como $f(x_1,\dots,x_n)$, donde $(x_1,\dots,x_n)=(x_1(p),\dots,x_n(p))$ son sus coordenadas por una carta que escojamos. 
\bigskip
...
\begin{definition}
    Sean $f:\mathrm{M}\to \mathbb{R}$ una función suave y $p\in M$ un punto crítico de f. Definimos $Hess_{p}f: T_{p}\mathrm{M}\to \mathbb{R}$ como la forma bilineal simétrica sobre el plano tangente a $\mathrm{M}$ en $p$ construida como sigue:
    \begin{quote}
        Tomamos $v,w\in T_{p}\mathrm{M}$ y sean $\tilde{v}$ y $\tilde{w}$ extensiones locales suyas a campos vectoriales. Entonces 
        \begin{equation*}
            Hess_{p}f(v,w)=\tilde{v}_{p}(\tilde{w}_{p}(f))
        \end{equation*}
...
\begin{remark}
    Un punto crítico $p\in M$ de $f$ suave es no degenerado $\iff$ La nulidad de $H$ sobre $T_pM$ es $0$. En efecto:
    \vspace{-2em}
    \begin{quote}
        \mbox{}\\[0.5em]
...
{\color{blue} ¿Ejemplos de puntos criticos degenerados y no degenerados?}
