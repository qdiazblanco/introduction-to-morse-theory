...
\begin{proof}[Demostración del Lema de Morse]
    ...
    Sabemos que, por ser la matriz $(H_{ij})$ no singular, los términos de la diagonal son todos no nulos. Luego podemos asumir $H_{rr}\neq 0$. Sea $g(u_1,\dots,u_n)=|H_{rr}(u_1,\dots,u_n)|^{1/2}$, es claro que existe un entorno $\mathcal{U}_2\subset \mathcal{U}_1$ de $0$ en el que es suave y no se anula. Definimos la siguiente función $V:M\to \mathbb{R}^n$ por componentes como
    \begin{equation*}
    \refstepcounter{pequation}
        \begin{cases}
    ...
    \label{eq2}
    \end{equation*}
    Se tiene que la matriz Jacobiana de $V$ en el $0$ es
    \begin{equation*}
        D_0V(u_1,\dots, u_n) = 
        \begin{pmatrix}
            1 & 0 & ~ & \dots & ~ & ~ & 0 \\
        ...
        \end{pmatrix},
    \end{equation*}
    y es no singular, basta restar a la $r$-ésima fila las demás multiplicadas por el término $\frac{\partial V_i}{\partial u_j}$ adecuado para que sea diagonal.

    Se sigue entonces del \textit{teorema de la función inversa} que $V$ es un difeomorfismo sobre un entorno más pequeño $\mathcal{U}_3\subset \mathcal{U}_2$ del $0$. Es decir, el par $(V,\mathcal{U}_3)$ es una carta de $M$ y, por tanto, un sistema de coordenadas local alrededor de $p$.
    ...
    donde el $\pm$ será $+$ si $H_{rr}$ es positivo y $-$ si es negativo. Así, es claro que \eqref{eq3} y \eqref{eq4} son equivalentes. Y desde aquí es fácil ver como \eqref{eq1} se convierte en 
    \begin{equation*}
        f = \pm (V_1)^2 \pm\dots\pm (V_{r-1})^2 + (V_r)^2  \pm \sum_{i,j > r} V_iV_j\tilde{H}_{ij}(V_1,\dots,V_n)
    \end{equation*}
    para unos nuevos términos $\tilde{H}_{ij}$ simétricos en función del nuevo sistema de coordenadas $V$.
