...
A su vez, también se verifica que $\partial_{k}\circ\partial_{k+1} = 0$ {\color{red} esto no veo claro por qué es}. Ahora que hemos visto como actuan los operadores de borde, los grupos de homología se construyen exactamente igual que en el caso simplicial.
...
Ahora, para los operadores de borde vayamos por partes. En primer lugar, para que tenga sentido definirlos debe ocurrir que $\partial_k$ lleve $C_k(A)$ a $C_{k-1}(A)$. En el caso de un espacio triangulado se puede demostrar que tomando un subespacio cualquiera podemos refinar la triangulación hasta que el subespacio quede triangulado unicamente por poliedros completos. Para CW-complejos el subespacio debe ser un subcomplejo, es decir una colección de celdas completas del espacio. En realidad, en ambos casos es la misma situación, la diferencia es que para un espacio triangulado podemos modificar los poliedros hasta tener un subcomplejo de la triangulación.
...
Demostrar esto es uno de los objetivos oficiales de la sección de homología relativa de \cite[p.~115]{hatcher2002} en la nos hemos basado para esta sección.
