...
\begin{example}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \end{equation*}
        donde para las dimensiones pares los grupos de cadenas son isomorfos a $\mathbb{Z}$ y para las impares a $\{0\}$. Esto significa que $Z_j = C_j$ y $B_j = \{0\}$ para todo $j$, luego los grupos de homología de $\mathbb{CP}^n$ quedan
        \begin{equation*}
            H_j(\mathbb{CP}^n) =
            \begin{cases}
                \mathbb{Z} &\text{ si } j \text{ es par},\\
                \{0\} &\text{ si } j \text{ toma otros valores}.
            \end{cases}
        \end{equation*}
    \end{enumerate}
\end{example}
