...
Vamos ahora con los dos lemas, ambos con demostración.
...
\begin{proof}
    ...
    Para la hipótesis de inducción supongamos que $M^a$ con $c_{i-1}<a<c_{i}$ tiene el tipo de homotopía de un CW-complejo $K$. Denotemos por $h:M^a\to K$ una equivalencia homotópica. Por el teorema \ref{pegadocelula_general}, para $\varepsilon$ suficientemente pequeño $M^{c_i+\varepsilon}$ tiene el tipo de homotopía de 
    \begin{equation*}
        M^{c_i - \varepsilon}\cup_{\varphi_1} e^{\lambda_1} \cup\dots\cup_{\varphi_j}
    \end{equation*}
    para ciertas funciones de pegado $\varphi_1,\dots,\varphi_j$. Además, por el teorema \ref{retractodef} existe una equivalencia homotópica $g: M^{c_i-\varepsilon}\to M^a$.

    Ahora, por el teorema de aproximación celular (\ref{aprox_celular})
