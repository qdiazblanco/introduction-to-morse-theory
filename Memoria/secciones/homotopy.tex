...
El último resultado corona la sección relacionando el tipo de homotopía de una variedad diferenciable con el de un CW-complejo cuyas celdas vienen definidas por los puntos críticos de una función de Morse. Su demostración necesitará del \textit{teorema de aproximación celular} cuya demostración se puede encontrar en \cite[p.~349]{hatcher2002} y de dos lemas acerca de un espacio topológico con una celda pegada que probaremos.
\vspace{0.5em}
...
\begin{proof}
    ...
    \begin{cases}
        ...
        h_s(tu) = \varphi_{((4-3s)t-1)(1-s)}(u) &~~\text{ si }~~ \frac{1}{4-3s} \le t \le \frac{2-s}{4-3s},~u\in \partial e^{\lambda} \\[4pt]
        h_s(tu) = \varphi_{(1-s)(2-2t)\frac{(2-s)-(4-3s)t}{2-4t}}(u),
        &~~\text{ si }~~  \frac{2-s}{4-3s} < t \le 1,~u\in \partial e^{\lambda}.
    \end{cases}
    \end{equation*}
    En el útlimo tramo dejamos una desigualdad estricta ya que para $t=1/2$ no está definida la función, $t$ solo puede tomar este valor para $s=0$, basta definir $h_0(u/2)=\varphi_1(u)$ para tener continuidad. 
    
    Es claro que para $s=0$ recuperamos $l\circ k$ y para $s=1$ se tiene la identidad de $X\cup_{\varphi_0} e^{\lambda}$. Que $h_s$ está bien definida se comprueba como en el caso de $k$.
...
\begin{proof}
    ...
    Sea $a\in\mathbb{R}\setminus\{c_i\}_{i\in \mathcal{C}}$, procedemos por inducción en la cantidad de valores críticos menores que $a$. 
    ...
    Para la hipótesis de inducción supongamos que $M^a$ con $c_i<a<c_{i+1}$ tiene el tipo de homotopía de un CW-complejo $K$. Denotemos por $h':M^a\to K$ una equivalencia homotópicas. Por el teorema \ref{pegadocelula_general}, para $\varepsilon$ suficientemente pequeño $M^{c_i+\varepsilon}$ tiene el tipo de homotopía de 
    \begin{equation*}
        M^{c_i - \varepsilon}\cup_{\varphi_1} e^{\lambda_1} \cup\dots\cup_{\varphi_j}
    \end{equation*}
    para ciertas funciones de pegado $\varphi_1,\dots,\varphi_j$.
